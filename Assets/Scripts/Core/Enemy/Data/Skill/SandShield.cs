...
[CreateAssetMenu(fileName = "SandShield", menuName = "Enemy/Skill/SandShield", order = 1)]
public class SandShield : SkillData
{
    ...
    {
        SandShield Instance = CreateInstance<SandShield>();
    ...
    IEnumerator ActivateSkill(Enemy enemy, IDamageable damageable)
    {
        ...
        Vector3 potentialPosition = enemy.transform.position;
        ...
        // else
        // {
        //     Debug.LogError("SandShield: No ground found");
        // }

        // GameObject skill = _skillPool.Get();
        // AreaDamage areaDamage = skill.GetComponentInChildren<AreaDamage>();

        GameObject skill = Instantiate(skillPrefab, potentialPosition, Quaternion.identity);
